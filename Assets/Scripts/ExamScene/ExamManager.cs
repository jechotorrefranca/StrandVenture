...
public class ExamManager : MonoBehaviour
{
    ...
    void EndExam()
    {
        ...
        foreach (var strand in strandScores.Keys)
        {
            ...
            PlayerPrefs.SetFloat($"{strand}_AvgTime", avgStrandTime);
            PlayerPrefs.SetInt($"{strand}_Questions", strandQuestionCounts[strand]);

            Debug.Log($"{strand}: Saved Score={strandScore:F1}%, AvgTime={avgStrandTime:F2}s, Questions={strandQuestionCounts[strand]}");
        }

        PlayerPrefs.Save();

        // --- Logging Summary ---
        Debug.Log($"✅ Exam Complete! Best Strand: {bestStrand}, Score: {bestScore:F1}%, Avg Time: {avgTime:F2}s");
        Debug.Log("──────────────────────────────");

        // --- Text-Based Graph (Score Distribution) ---
        ...
        foreach (var strand in strandScores.Keys)
        {
            // Calculate percentage of correct answers from this strand
            float percentage = totalCorrectAnswers > 0 ? ((float)strandScores[strand] / totalCorrectAnswers) * 100f : 0f;
            int barLength = Mathf.RoundToInt((percentage / 100f) * 20); // 20 chars max width
        ...
        Debug.Log("──────────────────────────────");
        Debug.Log($"🏆 Best Strand: {bestStrand} ({bestScore:F1}%)");
        Debug.Log("──────────────────────────────");
        ...
        if (bot != null)
            ...
            Debug.LogWarning("⚠️ No AptitudeBotController found in scene!");
    }
}